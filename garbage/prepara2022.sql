...
--
-- Host: 127.0.0.1
-- Tempo de geração: 26-Ago-2022 às 11:08
...
CREATE TABLE `turmas` (
  ...
  `ano_escolaridade` int(2) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
ALTER TABLE `alunos`
  ...
  ADD KEY `fk_AlunoTurma` (`id_turma`);
...
ALTER TABLE `turmas`
  ...
  ADD KEY `fk_TurmaEscola` (`id_escola`);
...
--
ALTER TABLE `turmas`
  MODIFY `id_turma` int(2) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `alunos`
  ADD CONSTRAINT `fk_AlunoTurma` FOREIGN KEY (`id_turma`) REFERENCES `turmas` (`id_turma`);

--
-- Limitadores para a tabela `turmas`
--
ALTER TABLE `turmas`
  ADD CONSTRAINT `fk_TurmaEscola` FOREIGN KEY (`id_escola`) REFERENCES `usuarios` (`id_escola`);
COMMIT;
